...
public class GameStage extends JPanel implements Runnable {
    ...
    // Hàm bắt đầu
    private void startGame(User user, String map){
        user.createPlayer();
        this.spawnMap  = loadMap.getLevel(map);
        this.map = this.spawnMap.map;
        this.spawnMap.findSpawnPoint();
    ...
    private void towerUpdate() {
        for (int i = 0 ; i < 22; i ++ ){
            for (int j =0 ; j < 13 ; j ++){
                if (towerMap[i][j] != null) towerAttack(i ,j);
            }
        }
    }
    ...
    private void towerAttack(int x, int y) {
        if (towerMap[x][y].target == null){
            if (towerMap[x][y].attackDelay >= towerMap[x][y].maxAttackDelay){
                EnemyMove currentEnemy = towerMap[x][y].calculateEnemy(enemyMap,x, y);
                if (currentEnemy != null ){
                    towerMap[x][y].towerAttack(currentEnemy);

                    towerMap[x][y].target = currentEnemy;
                    towerMap[x][y].attackTime = 0;
                    towerMap[x][y].attackDelay = 0;
                    System.out.println("enemy attack! health : " + currentEnemy.getHealth() + " x: " + x + " y : " + y);
                }
            }
            else {
    ...
    public void spawnEnemy(int x){
        for(int i = 0; i < enemyMap.length; i++){
            if(enemyMap[i] == null){
                enemyMap[i] = new EnemyMove(Enemy.enemyList[x], spawnMap.getSpawnPoint());
                break;
            }
        }
    }
    // Tính vị trí tháp trên map
    private void placeTower(int x, int y) throws CloneNotSupportedException {
        int xPos = x /40;
        int yPos = y /40;
    ...
    public class PressMouse{
        ...
        void updateMouse(MouseEvent e){
            ...
            // loại 2 ô 2 sniper
                ...
                System.out.println("You bought a tower for " + Tower.towerList[1].cost + "!");
                hand = 2;
            }
            ...
            //loại 3 ô 3 power
                ...
                System.out.println("You bought a tower for " + Tower.towerList[2].cost + "!");
                hand = 3;
            }
        }
